...
import { createProduct, updateProduct } from '@/api/product/actions'
import { type ProductFormSchema, productFormSchema } from '@/api/product/validations'
...
export const CreateUpdateProductModal = ({ product, ...props }: Props) => {
  ...
  const form = useForm<ProductFormSchema>({
    ...
    defaultValues: product ? { ...product, description: product.description ?? '' } : defaultValues,
  })

  useEffect(() => {
    form.reset(product ? { ...product, description: product.description ?? '' } : defaultValues)
  }, [product, form])
  ...
  const onSubmit = async (values: ProductFormSchema) => {
    try {
      ...
      queryClient.invalidateQueries({
        predicate: (query) =>
          ['products', 'productsToSale', 'productsToPurchase'].includes(
